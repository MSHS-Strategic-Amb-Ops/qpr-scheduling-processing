...
qpr_scheduling_query <- glue("CREATE TABLE PROD_QPR_SCHEDULING AS
                             SELECT k.*,
                             ...
                             CASE WHEN k.OFFICE_PROCEDURE_APPT_MAP IS NOT NULL THEN k.OFFICE_PROCEDURE_APPT_MAP
                                  ...
                                  WHEN k.OFFICE_PROCEDURE_APPT_MAP IS NULL AND k.OFFICE_PROCEDURE_PROV_MAP IS NULL THEN k.OFFICE_PROCEDURE_DEPT_MAP
                             ...
                             FROM
                             (SELECT c.*, d.CAMPUS, d.CAMPUS_SPECIALTY, d.CLINICAL_DEPT_DEPT_MAP,
                             ...
                             FROM
                             (SELECT APPT_DTTM, APPT_MADE_DTTM, CONTACT_DATE, DEPARTMENT_ID, DEPARTMENT_NAME, 
                             NPI, PROV_NAME_WID, PRC_NAME, PAT_ENC_CSN_ID, VISIT_GROUP_NUM, VISIT_METHOD,
                             DERIVED_STATUS_DESC, FINCLASS, VISITPLAN, VISITPAYOR,
                             ENC_CLOSED_CHARGE_STATUS, Y_ENC_CLOSE_TIME, DEP_RPT_GRP_SEVENTEEN,
                             LOS_NAME, ADD_LINE_1 AS PAT_ADD_LINE_1, ADD_LINE_2 AS PAT_ADD_LINE_2,
                             ...
                             WHERE EXTRACT(YEAR FROM APPT_MADE_DTTM) >= 2021) a
                             LEFT JOIN 
                             ...
                             ON e.DEPARTMENT_ID = f.DEPARTMENT_ID AND e.NPI = f.NPI) g
                             LEFT JOIN
                             (SELECT * FROM MASTER_AMB_MAPPING_APPT) h
                             ON g.CAMPUS_SPECIALTY_APPT_MAP = h.CLINICAL_DEPT_DEPT_MAP AND g.PRC_NAME = h.PRC_NAME) k
                             ")
